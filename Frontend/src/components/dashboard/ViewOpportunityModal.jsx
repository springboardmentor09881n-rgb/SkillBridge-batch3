...
const ViewOpportunityModal = ({ opportunity, onClose }) => {
    ...
    const handleMessageClick = async () => {
        ...
        if (!opportunity.organizationId) {
            alert("This opportunity doesn't have a valid Organizer ID attached, so you cannot message them.");
            return;
        }

        try {
            // Create a first message to jumpstart the conversation if it doesn't exist
            await fetch('http://localhost:5000/api/messages', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    senderId: user._id || user.id,
                    receiverId: opportunity.organizationId,
                    content: `Hello! I'm interested in your "${opportunity.title}" opportunity.`
                })
            });
            onClose();
            navigate('/Messages');
            ...
            console.error(error);
            alert("Network error trying to send the message.");
        }
    };
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 flex flex-col items-center">
                    ...
                    {user && user.userType === 'volunteer' && (
                        <button
                            ...
                            className="w-full mt-3 py-3 px-6 rounded-xl font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-200 transition-colors flex items-center justify-center gap-2"
                        >
